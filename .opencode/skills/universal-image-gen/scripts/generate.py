...
# API 端點
ANTIGRAVITY_API_URL = "http://192.168.1.159:8045/v1/messages"
...
def generate_antigravity(prompt, size="1280x720", quality="hd", n=1, images=None):
    """使用 Antigravity API 生成圖片"""
    ...
    response = requests.post(ANTIGRAVITY_API_URL, json=payload, headers=headers, timeout=120)
...
def extract_images_from_antigravity(response):
    """從 Antigravity API 響應中提取圖片"""
    images = []
    content = response.get("content", [])[0]
    text = content.get("text", "")

    import re
    matches = re.findall(r'data:image/jpeg;base64,([A-Za-z0-9+/=]+)', text)
