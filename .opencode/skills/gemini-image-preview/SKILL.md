...
description: 使用 AllAPI 提供的 Gemini 模型進行圖片生成，支援文生圖、圖生圖、多圖合成。提供兩個模型選項：gemini-3-pro-image-preview (NanoBanana Pro) 和 gemini-2.5-flash-image (Flash)
metadata:
  category: generation
  type: image
  languages:
...
# Gemini Image Preview - 圖片生成技能

使用 AllAPI 提供的 Gemini 模型進行圖片生成，支援文生圖、圖生圖、多圖合成。

## API 資訊

- **API 提供商**: AllAPI
- **模型選項**:
  - `gemini-3-pro-image-preview` (NanoBanana Pro) - 最強大的圖片生成模型
  - `gemini-2.5-flash-image` (Flash) - 快速圖片生成模型
- **Endpoint**: `POST /v1beta/models/{model-name}:generateContent`
- **Base URL**: `https://allapi.store/`
- **官方文檔**: https://ai.google.dev/gemini-api/docs/image-generation?hl=zh-cn
...
**Python**：
```bash
python3 .opencode/skills/gemini-image-preview/scripts/generate.py \
  "一隻可愛的貓咪在陽光下玩耍" \
  --ratio 16:9 \
  --size 4K
```
...
**Node.js**：
```bash
node .opencode/skills/gemini-image-preview/scripts/generate.js \
  "一隻可愛的貓咪在陽光下玩耍" \
  --ratio 16:9 \
  --size 4K
```
...
本技能使用 AllAPI 提供的 Gemini 3 Pro Image Preview API。Base URL 已設置為 `https://allapi.store/v1beta`。

如需獲取 AllAPI 的 API Key，請訪問 https://help.allapi.store/api-403555463
