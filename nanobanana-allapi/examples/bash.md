...
### 使用 Pro 版本
```bash
curl -X POST "https://allapi.store/v1beta/models/nanobanana-3-pro-image:generateContent?key=$API_KEY" \
...
### 使用 Flash 版本（快速）
```bash
curl -X POST "https://allapi.store/v1beta/models/nanobanana-2.5-flash-image:generateContent?key=$API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "contents": [
      {
